...
export type GrokFormRequestValues = z.infer<typeof grokSchema>;

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (values: z.infer<typeof grokSchema>) => void;
  defaultValues?: Partial<GrokFormRequestValues>;
}
...
export const GrokDialog = ({
  ...
  const form = useForm<z.infer<typeof grokSchema>>({
    resolver: zodResolver(grokSchema),
    defaultValues: {
      name: defaultValues?.name,
      credentialId: defaultValues?.credentialId,
      model: defaultValues?.model || AVAILABLE_GROK_MODELS[0],
  ...
  const handleSubmit = (values: z.infer<typeof grokSchema>) => {
    onSubmit(values);
    onOpenChange(false);
  };
        ...
        <Form {...form}>
          ...
          >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Grok Credential</FormLabel>
                  <Select
                ...
                <FormItem>
                  ...
                  >
                    ...
                    <SelectContent accessKey="">
                      {AVAILABLE_GROK_MODELS.map((model) => (
                  ...
                  <FormMessage />
                  <FormDescription>
                    The Grok Model to use for the request.
                  </FormDescription>
                </FormItem>
              )}
            />
